import {
    ManageAccountsOutlined,
    EditOutlined,
    LocationOnOutlined,
    WorkOutlineOutlined
} from "@mui/icons-material";


import { Box, Typography , Divider, useTheme } from "@mui/material";
import UserImage from "../../components/UserImage";
import FlexBetween from "../../components/FlexBetween";
import WidgetWrapper from "../../components/WidgetWrapper";
import AppConfig from "../../app.config"; 
import Mocks from "../../mocks"; 
import { retryHandler } from "../../utils/common";


import { useSelector } from "react-redux";
import { useEffect , useState } from "react";
import { useNavigate } from "react-router-dom";

const UserWidget = ({ userId, picturePath }) => {

    const [ user, setUser ] = useState(null);
    const { palette } = useTheme();
    console.log("palete ", palette);
    const navigate = useNavigate();
    const token = useSelector((state) => state.token);
    const dark =  palette.neutral?.dark;
    const medium = palette.neutral?.medium;
    const main = palette.neutral?.main;

    const { assets, isMockMode } = AppConfig;
    const { user: UserMockData } = Mocks;

    console.log("picturePath", picturePath);

    const getUser =  async (userId) => {

        console.log("called endpoint", userId);
        // const response = await fetch(`http://localhost:3001/users/${userId}`, {
        //     method: 'GET',
        //     headers: { Authorization: `Bearer ${token}`}
        // });
        // const data = await response.json();
        // setUser(data)
    }

    useEffect(() => {
        if(isMockMode) {
            setUser(UserMockData)
        }else{
            retryHandler( getUser(userId), 3)
        }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [ ])

    if(!user) {
        return null;
    }

    const {
        firstName, 
        lastName,
        location,
        occupation,
        viewedProfile,
        impressions,
        friends
    } = user;

    return (
        <WidgetWrapper>

            <FlexBetween
            gap="0.5rem"
            flexDirection="column"
            pb="1.1rem"
            onClick={ () => navigate(`/profile/${userId}`)}
            >
                <Box gap="1rem" display="flex" width="1" justifyContent="space-between" alignItems="center">
                    <UserImage  image={ picturePath ?? assets.defaultImage }/>
                    <Box>
                        <Typography
                        variant="h4"
                        color={ dark}
                        fontWeight="500"
                        sx={{
                            "&:hover": {
                                color: palette.primary.light,
                                cursor: "pointer"
                            }
                        }}
                        >
                            {firstName} {lastName} 
                        </Typography>
                        <Typography color={medium}>{ friends?.length ?? 0 } Friends</Typography>
                    </Box>
                    <ManageAccountsOutlined/>
                </Box>


                </FlexBetween> 

                <Divider/>

                { /**SECOND ROW */}
                <Box p="1rem 0" width="1">
                    <Box display="flex"  gap="1rem" mb="0.5rem">
                        <LocationOnOutlined fontSize="medium" sx={{ color: main }}/>
                        <Typography color={medium}>{location}</Typography>
                    </Box>
                    <Box display="flex" alignItems="center" gap="1rem" mb="0,5rem">
                        <WorkOutlineOutlined fontSize="medium" sx={{ color: main }}/>
                        <Typography color={medium}>{occupation}</Typography>
                    </Box>
                </Box>
                
                <Divider/>

                { /**THIRD ROW */}
                <Box p="1rem 0" width="1">
                   <FlexBetween mb="1rem">
                        <Typography color={medium}>Who's viewd your profile</Typography>
                        <Typography color={main} fontWeight="500">{viewedProfile}</Typography>
                   </FlexBetween>
                   <FlexBetween mb="1rem">
                        <Typography color={medium}>Impressions of your posts</Typography>
                        <Typography color={main} fontWeight="500">{impressions}</Typography>
                   </FlexBetween>
                </Box>


        </WidgetWrapper>
    )

}
export default UserWidget;